...
{
    internal ServerInfo(bool? autoDetect, string? autoConfigUrl, int? proxyEnable, string? proxyServer,
    ...
    internal Dictionary<ServerProtocolType, HttpSystemProxyValue>? Proxies { get; }
    ...
    internal static ServerProtocolType? ParseProtocolType(string protocolTypeStr)
    {
        if (protocolTypeStr == null) return null;

        ServerProtocolType? protocolType = null;
        if (protocolTypeStr.Equals(T2Proxy.ProxyServer.UriSchemeHttp, StringComparison.InvariantCultureIgnoreCase))
            protocolType = ServerProtocolType.Http;
        else if (protocolTypeStr.Equals(T2Proxy.ProxyServer.UriSchemeHttps,
                     StringComparison.InvariantCultureIgnoreCase))
            protocolType = ServerProtocolType.Https;
