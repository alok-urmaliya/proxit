...
internal sealed class WinHttpWebProxyFinder : IDisposable
{
    ...
    public ServerInfo? ProxyInfo { get; internal set; }
    ...
    public IExternalServer? GetProxy(Uri destination)
    {
        if (GetAutoProxies(destination, out var proxies))
        {
            ...
            var systemProxy = new ExternalServer(proxyStr, port);
        ...
        var protocolType = ServerInfo.ParseProtocolType(destination.Scheme);
        if (protocolType.HasValue)
        {
            ...
            {
                var systemProxy = new ExternalServer(value!.HostName, value.Port);
                return systemProxy;
            }
        }
    ...
    private ServerInfo GetProxyInfo()
    {
        ...
        try
        {
            ...
            else
            {
                ...
                result = new ServerInfo(true, null, null, null, null);
            }
